...
> 개인 프로젝트로 스프링 부트를 이용해 백엔드 개발을 했습니다.
> 금융 거래의 흐름과 결제 시스템 구조를 파악하고자 PG 사(카카오, 토스)의 결제 API를 연동한 결제 시스템을 구현했습니다.
> 기술적으로는 scale-out한 환경에서의 신뢰성 있는 결제 시스템을 제공하는 것을 목적으로 개발했습니다.
...
│   │   │   │  │  └──  CompensationStepExecutionListener
│   │   │   │  │  │  ├──  correction
│   │   │   │  │  │  │  └──  CorrectionFileWriter
│   │   │   │  │  │  ├──  enums
│   │   │   │  │  │  │  └──  PGManageName
│   │   │   │  │  │  ├──  exception
│   │   │   │  │  │  │  └──  CompensationFileNotFoundException
│   │   │   │  │  │  ├──  file
│   │   │   │  │  │  │  ├──  controller
│   │   │   │  │  │  │  │  └──  CompensationFileController
│   │   │   │  │  │  │  ├──  service
...
│   ├── log4j2-local.xml
│   ├── log4j2-prod.xml
│   ├── payment_kakao.csv # 배치 시스템에 필요한 예시 엑셀 파일, 주기적으로 PG 사의 API와 통신해 엑셀 파일을 가져와 데이터베이스의 데이터와 비교할 것이라 가정
└   └── payment_toss.csv
```
...
1) 회원가입 & 로그인
  - Redis 기반 세션 관리로 확장 가능한 인증 시스템 구축
...
상용 애플리케이션을 참고하는 과정에서 결제 완료 후 서버에서 승인 응답을 받지 못했는데 실제로 결제는 완료된 경우를 발견했습니다. 이로 인해 서버와 PG 사 간 결제 데이터가 일치하지 않을 수 있는 가능성을 발견했습니다. 타임아웃 설정으로 인해 PG 사는 결제 승인을 보내지만 서버가 응답을 받지 못해 결제 실패로 처리할 것이고, 이를 해결하고자 개선이 필요하다고 판단했습니다. Spring Batch 스케줄링을 도입하여 주기적으로 PG 사의 결제 승인 데이터(정산API 사용)와 서버 데이터를 비교하는 시스템을 구현했습니다. 결제 정보가 일치하지 않을 경우 수정되도록 하여 데이터 정합성을 보장했습니다. 또한 대규모 데이터 처리 성능을 향상시키기 위해 멀티스레딩과 파티셔닝을 적용해 병렬 처리를 가능하게 했습니다. 
...
1. 스케쥴링을 위해 spring batch를 사용했지만 대용량 데이터 처리시 성능 이슈가 생길 수 있다는 것을 알았습니다. 추후 기회가 된다면 성능 개선을 해보고 싶습니다.
2. 통합 테스트 작성 시 성공 케이스만 작성을 했는데 실패 케이스까지 작성을 해줘야한다는 것을 알게 됐습니다. 추후 기회가 된다면 적용하고 싶습니다.
