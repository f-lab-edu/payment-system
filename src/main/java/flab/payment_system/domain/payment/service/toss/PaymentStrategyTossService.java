...
import flab.payment_system.common.util.DateUtil;
...
import flab.payment_system.domain.payment.enums.PaymentTossEndpoint;
...
public class PaymentStrategyTossService implements PaymentStrategy {
	...
	public PaymentApprovalDto approvePayment(String pgToken, Long orderId, Long userId,
		...
		PaymentTossDtoImpl paymentTossDto = paymentTossClient.approvePayment(tossHost + PaymentTossEndpoint.PAYMENT.getEndpoint()
			+ PaymentTossEndpoint.APPROVE.getEndpoint(), body);
	...
	public PaymentCancelDto cancelPayment(OrderCancelDto orderCancelDto) {
		...
		return paymentTossClient.cancelPayment(
			tossHost + PaymentTossEndpoint.PAYMENT.getEndpoint() + "/" + orderCancelDto.paymentKey() + PaymentTossEndpoint.CANCEL, body);
	}
	...
	public PaymentOrderDetailDto getOrderDetail(String paymentKey) {
		...
		return paymentTossClient.getOrderDetail(tossHost + PaymentTossEndpoint.PAYMENT.getEndpoint() + "/" + paymentKey, requestEntity);
	}
	...
	public Settlement[] getSettlementList() {
		...
		HttpHeaders headers = paymentTossRequestBodyFactory.getHeaders();
		HttpEntity<Void> requestEntity = new HttpEntity<>(headers);
		String url = tossHost + PaymentTossEndpoint.SETTLEMENT.getEndpoint()
			+ "?startDate=" + yesterdayDate + "&endDate=" + yesterdayDate;
		return paymentTossClient.getSettlementList(url, requestEntity);
	}
}
